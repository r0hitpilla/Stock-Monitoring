...
``extract_variants``) and :meth:`BigBasketProvider.check_availability_from_page`
...
class BigBasketProvider(BaseRetailProvider):
    """Retail provider adapter for BigBasket (bigbasket.com)."""
    ...
    async def initialize(self, location: LocationContext) -> None:
        """Launch a browser and set the delivery location on BigBasket.
    ...
    async def search_product(self, keyword: str) -> list[ProviderProductResult]:
        """Search BigBasket for products matching a keyword.
        ...
        try:
            await page.goto(
                f"{BASE_URL}/ps/?q={keyword}",
                wait_until="domcontentloaded",
                timeout=30000,
            )
            await page.wait_for_selector(
                BIGBASKET_SELECTORS["search_result_card"], timeout=10000
            )
            cards = await page.query_selector_all(
                BIGBASKET_SELECTORS["search_result_card"]
            )
    ...
    async def get_product(self, product_url: str) -> ProviderProductResult:
        """Fetch and extract full product details from a BigBasket product page.
    ...
    async def health_check(self) -> bool:
        """Check whether the provider's browser session can reach BigBasket.
