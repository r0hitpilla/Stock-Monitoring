...
``extract_variants``) and :meth:`BlinkitProvider.check_availability_from_page`
...
class BlinkitProvider(BaseRetailProvider):
    """Retail provider adapter for Blinkit (blinkit.com)."""
    ...
    async def initialize(self, location: LocationContext) -> None:
        """Launch a browser and set the delivery location on Blinkit.
    ...
    async def search_product(self, keyword: str) -> list[ProviderProductResult]:
        """Search Blinkit for products matching a keyword.
        ...
        try:
            await page.goto(
                f"{BASE_URL}/s/?q={keyword}",
                wait_until="domcontentloaded",
                timeout=30000,
            )
            await page.wait_for_selector(
                BLINKIT_SELECTORS["search_result_card"], timeout=10000
            )
            cards = await page.query_selector_all(
                BLINKIT_SELECTORS["search_result_card"]
            )
    ...
    async def get_product(self, product_url: str) -> ProviderProductResult:
        """Fetch and extract full product details from a Blinkit product page.
    ...
    async def health_check(self) -> bool:
        """Check whether the provider's browser session can reach Blinkit.
