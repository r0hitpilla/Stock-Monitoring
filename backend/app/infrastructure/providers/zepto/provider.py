...
``extract_variants``) and :meth:`ZeptoProvider.check_availability_from_page`
...
class ZeptoProvider(BaseRetailProvider):
    """Retail provider adapter for Zepto (zeptonow.com)."""
    ...
    async def initialize(self, location: LocationContext) -> None:
        """Launch a browser and set the delivery location on Zepto.
    ...
    async def search_product(self, keyword: str) -> list[ProviderProductResult]:
        """Search Zepto for products matching a keyword.
        ...
        try:
            ...
            await page.wait_for_selector(
                ZEPTO_SELECTORS["search_result_card"], timeout=10000
            )
    ...
    async def get_product(self, product_url: str) -> ProviderProductResult:
        """Fetch and extract full product details from a Zepto product page.
    ...
    async def health_check(self) -> bool:
        """Check whether the provider's browser session can reach Zepto.
